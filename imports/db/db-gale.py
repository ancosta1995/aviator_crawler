...
def update_state(name: str, value: str):
    ...
    # OPÇÃO 1: INSERT OR REPLACE (mais simples)
    cursor.execute("""
        INSERT OR REPLACE INTO states (name, value) 
        VALUES (?, ?)
    """, (name, value))
    
    # OPÇÃO 2: Se preferir manter a lógica de UPDATE/INSERT separada:
    # cursor.execute("UPDATE states SET value = ? WHERE name = ?", (value, name))
    # if cursor.rowcount == 0:  # Se nenhuma linha foi afetada
    #     cursor.execute("INSERT INTO states (name, value) VALUES (?, ?)", (name, value))

def get_state(name: str):
    """
    Retorna o value da tabela states pelo name.
    Retorna string vazia se não existir (em vez de None).
    """
    cursor = conn.cursor()
    cursor.execute("SELECT value FROM states WHERE name = ?", (name,))
    result = cursor.fetchone()
    return result[0] if result else ""  # Retorna string vazia em vez de None
